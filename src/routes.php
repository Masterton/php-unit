...
$container->get('globals')->set('appRouteList', []);
    ...
    foreach ($urls as $url => $action) {
        foreach ($action as $method => $content) {
            ...
            if(isset($id)) {
                ...
                if(array_key_exists('app', $content)) {
                    $is_app = $content['app'];
                    if($is_app === true) {
                        $container->get('globals')->item_push('appRouteList', $name);
                    }
                }
                ...
                if($settings['debug'] && array_key_exists('op_group',$content)){
                    ...
                    $container->get('globals')->item_push('op_group',$op_item);
                }
            }
            $i += 1;
        }
